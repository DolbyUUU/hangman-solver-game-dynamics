...
def load_model_and_tokenizer(use_bert_large=False):
    ...
    if use_bert_large:
        ...
        model_path = "./bert-base-ver1/hangman_bert_base"
...
def adjust_probabilities(probabilities, guessed_correct, guessed_incorrect, tokenizer):
    ...
    """
    exclude_ids = set(tokenizer.convert_tokens_to_ids(letter) for letter in guessed_correct.union(guessed_incorrect))
...
def predict_masked_character(
    ...
    # Tokenize and encode tokens, and prepare for model input
    encoding = tokenizer(
        tokens,
    ...
    letter_probs = {letter: adjusted_probs[tokenizer.vocab[letter]].item() for letter in string.ascii_lowercase}
    ...
    best_letter = tokenizer.convert_ids_to_tokens(best_idx)
